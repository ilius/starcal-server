	...
	routeGroups = append(routeGroups, RouteGroup{
		...
		Map: RouteMap{
			"AddDailyNote": {
				Method:      "POST",
				Pattern:     "",
				HandlerFunc: authWrap(AddDailyNote),
			},
			"GetDailyNote": {
				Method:      "GET",
				Pattern:     "{eventId}",
				HandlerFunc: authWrap(GetDailyNote),
			},
			"UpdateDailyNote": {
				Method:      "PUT",
				Pattern:     "{eventId}",
				HandlerFunc: authWrap(UpdateDailyNote),
			},
			"PatchDailyNote": {
				Method:      "PATCH",
				Pattern:     "{eventId}",
				HandlerFunc: authWrap(PatchDailyNote),
			},
			"MergeDailyNote": {
				Method:      "POST",
				Pattern:     "{eventId}/merge",
				HandlerFunc: authWrap(MergeDailyNote),
			},
...
func MergeDailyNote(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()
	w.Header().Set("Content-Type", "application/json; charset=UTF-8")
	userModel := CheckAuthGetUserModel(w, r)
	if userModel == nil {
