	...
	routeGroups = append(routeGroups, RouteGroup{
		...
		Map: RouteMap{
			"AddCustom": {
				Method:      "POST",
				Pattern:     "",
				HandlerFunc: authWrap(AddCustom),
			},
			"GetCustom": {
				Method:      "GET",
				Pattern:     "{eventId}",
				HandlerFunc: authWrap(GetCustom),
			},
			"UpdateCustom": {
				Method:      "PUT",
				Pattern:     "{eventId}",
				HandlerFunc: authWrap(UpdateCustom),
			},
			"PatchCustom": {
				Method:      "PATCH",
				Pattern:     "{eventId}",
				HandlerFunc: authWrap(PatchCustom),
			},
			"MergeCustom": {
				Method:      "POST",
				Pattern:     "{eventId}/merge",
				HandlerFunc: authWrap(MergeCustom),
			},
...
func MergeCustom(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()
	w.Header().Set("Content-Type", "application/json; charset=UTF-8")
	userModel := CheckAuthGetUserModel(w, r)
	if userModel == nil {
